...
function updateActiveTab(newTab) {
    if (activeTab != null && activeTab.ID == newTab.ID) {
        ...
        document
            .getElementById(activeTab.ID + '-content')
...
class AddNewOfficers {
    ...
    static update_checkbox(element) {
        let confirmIconElement =
            element.parentElement.getElementsByClassName('confirm-icon')[0];
        let closeElement =
    ...
    static async onSubmit(element) {
        ...
        for (let widget of widgetList) {
            let status = widget.getElementsByClassName(
                'add-new-officer-status'
            )[0];
            ...
            let computingId = widget.getElementsByClassName(
                'add-new-officer-computing-id'
            )[0].value;
            let position = widget.getElementsByClassName(
                'add-new-officer-position'
            )[0].value;
            let startDate = widget.getElementsByClassName(
                'add-new-officer-start-date'
            )[0].value;
            bodyObjectList.push({
        ...
        if (response.status == 500 || response.status == 422) {
            descText.innerHTML =
                'failed to upload data, with http error ' + response.status;
...
class ViewOfficers {
    ...
    static async updateTable() {
        let response = await fetch(
            '/api/officers/all?view_only_filled_in=false',
            {
                method: 'GET'
            }
        );
        ...
        if (response.status == 500 || response.status == 422) {
            descText.innerHTML =
                'failed to get data, with error ' + response.status;
        } else if (response.status != 200) {
            descText.innerHTML =
        ...
        document.getElementById('view-officers-table').style.overflowX =
            'scroll';
